  ...
  <div class="app-container">
    <div class="container-form-header">
      <div>
        ...
        <span style="font-size: 14px;">Quản lý Danh mục</span>
      </div>
    ...
    <div class="container-form-table">
      ...
      <el-dialog
        width="30%"
        :title="category.id ? 'Chỉnh sửa' : 'Thêm mới'"
      ...
      >
        ...
        >
          ...
          <el-form-item label="Trạng thái" prop="status">
            <el-select v-model="category.status" style="width: 100%;" placeholder="">
              <el-option
                v-for="s in listStatus"
                v-if="s.value !== 'ALL'"
                :key="s.value"
          ...
          <el-form-item v-if="category.id && category.updatedAt && category.updatedBy" style="margin-bottom: 0;">
            <span>Thời gian cập nhật gần nhất: {{ moment(category.updatedAt).format('HH:mm:ss DD-MM-YYYY') }} (bởi: {{ category.updatedBy }})</span>
          </el-form-item> -->
        </el-form>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="edit()">
            {{ category.id ? 'Chỉnh sửa' : 'Thêm mới' }}
          </el-button>
        </div>
      </el-dialog>
    </div>
...
<script>
import { categoryGetAllPage, categoryGetById, categoryCreateOrUpdate } from '@/api/category'
...
export default {
  name: 'ProductManagementCategoryListPage',
  ...
  methods: {
    getList() {
      ...
      categoryGetAllPage(this.listQuery).then(res => {
        setTimeout(() => {
        ...
        if (valid) {
          if (this.category.id) {
            this.$confirm('Bạn xác nhận cập nhật?', 'Xác nhận cập nhật', {
              ...
              categoryCreateOrUpdate(this.category).then(res => {
                if (res && res.code === ResponseCode.CODE_SUCCESS) {
                  this.$message({
                    showClose: true,
      ...
      categoryGetById(id).then(res1 => {
        if (res1 && res1.code === ResponseCode.CODE_SUCCESS) {
          this.$confirm('Chuyển trạng thái thành: "Ngừng kinh doanh", bạn chắc chắn chứ?', 'Xác nhận xóa', {
            ...
            categoryCreateOrUpdate(res1.data).then(res2 => {
