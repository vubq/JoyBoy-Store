  ...
  <div class="app-container">
    <div class="container-form-header">
      <div>
        ...
        <span style="font-size: 14px;">Quản lý Chất liệu</span>
      </div>
    ...
    <div class="container-form-table">
      ...
      <el-dialog
        width="30%"
        :title="material.id ? 'Chỉnh sửa' : 'Thêm mới'"
      ...
      >
        ...
        >
          ...
          <el-form-item label="Trạng thái" prop="status">
            <el-select v-model="material.status" style="width: 100%;" placeholder="">
              <el-option
                v-for="s in listStatus"
                v-if="s.value !== 'ALL'"
                :key="s.value"
          ...
          <el-form-item v-if="material.id && material.updatedAt && material.updatedBy" style="margin-bottom: 0;">
            <span>Thời gian cập nhật gần nhất: {{ moment(material.updatedAt).format('HH:mm:ss DD-MM-YYYY') }} (bởi: {{ material.updatedBy }})</span>
          </el-form-item> -->
        </el-form>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="edit()">
            {{ material.id ? 'Chỉnh sửa' : 'Thêm mới' }}
          </el-button>
        </div>
      </el-dialog>
    </div>
...
<script>
import { materialGetAllPage, materialGetById, materialCreateOrUpdate } from '@/api/material'
...
export default {
  name: 'ProductManagementMaterialListPage',
  ...
  methods: {
    getList() {
      ...
      materialGetAllPage(this.listQuery).then(res => {
        setTimeout(() => {
        ...
        if (valid) {
          if (this.material.id) {
            this.$confirm('Bạn xác nhận cập nhật?', 'Xác nhận cập nhật', {
              ...
              materialCreateOrUpdate(this.material).then(res => {
                if (res && res.code === ResponseCode.CODE_SUCCESS) {
                  this.$message({
                    showClose: true,
      ...
      materialGetById(id).then(res1 => {
        if (res1 && res1.code === ResponseCode.CODE_SUCCESS) {
          this.$confirm('Chuyển trạng thái thành: "Ngừng kinh doanh", bạn chắc chắn chứ?', 'Xác nhận xóa', {
            ...
            materialCreateOrUpdate(res1.data).then(res2 => {
